...
add_action( 'widgets_init', 'vibe_projects_incomplete_tasks_by_project_widget' );
function vibe_projects_incomplete_tasks_by_project_widget() {
    register_widget('vibe_projects_incomplete_tasks_by_project');
}

class vibe_projects_incomplete_tasks_by_project extends WP_Widget {

    protected static $did_script = false;
    /** constructor -- name this the same as the class above */
    function __construct() {
        $widget_ops = array( 'classname' => 'vibe_projects_incomplete_tasks_by_project', 'description' => __('Incomplete Tasks by Project', 'vibe-projects') );
        $control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'vibe_projects_incomplete_tasks_by_project' );
        parent::__construct( 'vibe_projects_incomplete_tasks_by_project', __('DASHBOARD : Incomplete Tasks by Project', 'vibe-projects'), $widget_ops, $control_ops );
    ...
    function enqueue_scripts(){
        ...
        if(apply_filters('vibebp_enqueue_profile_script',false)){
            ...
            wp_enqueue_script('vibe_projects_incomplete_tasks_by_project',plugins_url('../../assets/js/incompletetasks.js',__FILE__), array('wp-element', 'wp-data','chartjs-js'), VIBEPROJECTS_VERSION,true);
            //wp_enqueue_style('vibe_projects_incomplete_tasks_by_project',plugins_url('../../assets/css/incompletetasks.css',__FILE__),array(),VIBEPROJECTS_VERSION);
            
            wp_localize_script('vibe_projects_incomplete_tasks_by_project', 'incomplete_tasks', array(
                'settings' => array(
    ...
    function add_custom_script($x){
        $x[] = 'vibe_projects_incomplete_tasks_by_project';
        return $x;
    }
    ...
    function widget( $args, $instance ) {
        ...
        echo '<div id="vibe_projects_incomplete_tasks_by_project"></div>';
    ...
    function form($args) {  
        $defaults = array( 
            'title'  => __('Incomplete Tasks by Project','vibe-projects'),
