...
class VIBE_Options{
	...
	*/
	function __construct($sections = array(), $args = array(), $extra_tabs = array()){
		...
		$defaults['menu_title'] = __('Options', 'micronet');
		$defaults['page_icon'] = 'icon-themes';
		$defaults['page_title'] = __('Options', 'micronet');
		...
		$defaults['footer_credit'] = '<span id="footer-thankyou">'.__('Options Panel created using', 'micronet').' the <a href="'.$this->framework_url.'" target="_blank">VIBE Theme Options Framework</a> Version '.$this->framework_version.'</span>';
	...
	*/
	function show($opt_name, $default = ''){
		...
		if(!is_array($option) && $option != ''){
			echo vibe_sanitizer($option);
		}elseif($default != ''){
			echo vibe_sanitizer($default);
		}
	}//function
	...
	*/
	function _enqueue(){
		...
		wp_localize_script('vibe-opts-js', 'vibe_opts', array('reset_confirm' => __('Are you sure? Resetting will loose all custom values.', 'micronet'), 'opt_name' => $this->args['opt_name']));
		...
		echo '<div class="wrap vibe-options-wrap">';
			...
			echo '<form method="post" action="options.php" enctype="multipart/form-data" id="vibe-opts-form-wrapper">';
				...
				echo '</div>';
					...
					if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('vibe-opts-saved') == '1'){
						if(isset($this->options['imported']) && $this->options['imported'] == 1){
							echo '<div id="vibe-opts-imported">'.apply_filters('vibe-opts-imported-text-'.$this->args['opt_name'], __('<strong>Settings Imported!</strong>', 'micronet')).'</div>';
						}else{
							echo '<div id="vibe-opts-save">'.apply_filters('vibe-opts-saved-text-'.$this->args['opt_name'], __('<strong>Settings Saved!</strong>', 'micronet')).'</div>';
						}
						delete_transient('vibe-opts-saved');
					}
					echo '<div id="vibe-opts-save-warn">'.apply_filters('vibe-opts-changed-text-'.$this->args['opt_name'], __('<strong>Settings have changed!, you should save them!</strong>', 'micronet')).'</div>';
					echo '<div id="vibe-opts-field-errors">'.__('<strong><span></span> error(s) were found!</strong>', 'micronet').'</div>';
					
					echo '<div id="vibe-opts-field-warnings">'.__('<strong><span></span> warning(s) were found!</strong>', 'micronet').'</div>';
					...
					echo '<ul id="vibe-opts-group-menu">';
						...
						if(true === $this->args['show_import_export']){
							echo '<li id="import_export_default_section_group_li" class="vibe-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="vibe-opts-group-tab-link-a" data-rel="import_export_default"><i class="icon-white icon-refresh"></i> <span>'.__('Import / Export', 'micronet').'</span></a>';
							echo '</li>';
							echo '<li class="divide">&nbsp;</li>';
						}//if
						...
						if(true === $this->args['dev_mode']){
							echo '<li id="dev_mode_default_section_group_li" class="vibe-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="vibe-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default"><i class="icon-white icon-info-sign"></i> <span>'.__('Dev Mode Info', 'micronet').'</span></a>';
							echo '</li>';
						}//if
				...
				echo '<div id="vibe-opts-main">';
					...
					if(true === $this->args['show_import_export']){
						echo '<div id="import_export_default_section_group'.'" class="vibe-opts-group-tab">';
							echo '<h3>'.__('Import / Export Options', 'micronet').'</h3>';
							
							echo '<h4>'.__('Import Options', 'micronet').'</h4>';
							
							echo '<p><a href="javascript:void(0);" id="vibe-opts-import-code-button" class="button-secondary">'.__('Import from file','micronet').'</a> <a href="javascript:void(0);" id="vibe-opts-import-link-button" class="button-secondary">'.__('Import from URL','micronet').'</a></p>';
							
							echo '<div id="vibe-opts-import-code-wrapper">';
							
								echo '<div class="vibe-opts-section-desc">';
				
									echo '<p class="description" id="import-code-description">'.apply_filters('vibe-opts-import-file-description',__('Input your backup file below and hit Import to restore your sites options from a backup.', 'micronet')).'</p>';
							...
							echo '<div id="vibe-opts-import-link-wrapper">';
								...
								echo '<div class="vibe-opts-section-desc">';
									...
									echo '<p class="description" id="import-link-description">'.apply_filters('vibe-opts-import-link-description',__('Input the URL to another sites options set and hit Import to load the options from that site.', 'micronet')).'</p>';
							...
							echo '<p id="vibe-opts-import-action"><input type="submit" id="vibe-opts-import" name="'.$this->args['opt_name'].'[import]" class="button-primary" value="'.__('Import', 'micronet').'"> <span>'.apply_filters('vibe-opts-import-warning', __('WARNING! This will overwrite any existing options, please proceed with caution!', 'micronet')).'</span></p>';
							echo '<div id="import_divide"></div>';
							
							echo '<h4>'.__('Export Options', 'micronet').'</h4>';
							echo '<div class="vibe-opts-section-desc">';
					...
					foreach($this->extra_tabs as $k => $tab){
						...
						echo vibe_sanitizer($tab['content']);
						echo '</div>';
					}
					...
					if(true === $this->args['dev_mode']){
						echo '<div id="dev_mode_default_section_group'.'" class="vibe-opts-group-tab">';
							echo '<h3>'.__('Dev Mode Info', 'micronet').'</h3>';
							echo '<div class="vibe-opts-section-desc">';
				...
				echo '<div id="vibe-opts-footer">';
					...
					submit_button('', 'vibe-button vibe-save', '', false);
					submit_button(__('Reset to Defaults', 'micronet'), 'vibe-button', $this->args['opt_name'].'[defaults]', false);
					echo '<div class="clear"></div><!--clearfix-->';
				echo '</div>';
