...
class Vibe_Options_text_upload extends Vibe_Options{	
	...
	*/
	function render(){
		...
		echo '<div class="field_text_upload_textarea" id="text_'.$this->field['id'].'" '.(filter_var($this->value, FILTER_VALIDATE_URL)?'style="display:none"':'').'>'._x('Raw Text Replacement for Image','options panel upload box','micronet').'<textarea style="display:block;" id="textarea_'.$this->field['id'].'" 
		...
		'._x('Uploaded Image','options panel upload box','micronet').'
		...
		echo ' <a href="javascript:void(0);" class="vibe-opts-upload button-secondary"'.$upload.' rel-id="'.$this->field['id'].'"  data-title="'.$this->field['title'].'"  data-save="#input_img_'.$this->field['id'].'" data-target="#vibe-opts-screenshot-'.$this->field['id'].'">'.__('Browse', 'micronet').'</a>';
		echo ' <a href="javascript:void(0);" class="vibe-opts-upload-remove"'.$remove.' rel-id="'.$this->field['id'].'">'.__('Remove Upload', 'micronet').'</a>
		</div></div>';
