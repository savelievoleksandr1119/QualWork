...
class Vibe_Options_multi_social extends Vibe_Options{	
	...
	*/
	function render(){
            ...
            foreach($this->value['social'] as $k=>$value){ 
                    ...
                    foreach($social_array as $key=>$label){
                        echo '<option value="'.$key.'" '.(($key == $value)?'selected':'').'>'.$label.'</option>';
                                                ...
                                                <a href="javascript:void(0);" class="vibe-opts-multi-social-remove">'.__('Remove', 'micronet').'</a>
                                             </li>';
        ...
        echo '</select>
                                                <input type="text" id="'.$this->field['id'].'[url]" name="" placeholder="'.__('Enter Full URL of the page','micronet').'" value="" class="'.$class.'" rel-name="'.$this->args['opt_name'].'['.$this->field['id'].'][url][]" /> 
                                                    <a href="javascript:void(0);" class="vibe-opts-multi-social-remove">'.__('Remove', 'micronet').'</a>
                                                </li>';
        
        echo '</ul>';
        
        echo '<a href="javascript:void(0);" class="vibe-btn green vibe-opts-multi-social-add " rel-id="'.$this->field['id'].'-ul">'.__('Add Social Icons', 'micronet').'</a><br/>';
