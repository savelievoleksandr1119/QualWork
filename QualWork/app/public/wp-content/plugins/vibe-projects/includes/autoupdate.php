...
class Vibe_Projects_Auto_Update{
	...
	public function __construct( $config = array() ) {
		...
		if ( !empty( $this->config['base'] ) && !empty ( $this->config['repo_uri'] ) && !empty ( $this->config['repo_slug'] ) ){
			...
			if ( is_admin() ) {
				...
				//add_filter( 'get_site_transient_update_plugins', array( $this, 'transient_update_plugins' ) );
	...
	public function updater_data(){
		...
		if ( empty( $key ) && true === $this->config['dashboard'] ){
			$widget_id = 'ahp_' . $slug . '_activation_key';
			$widget_option = get_option( $widget_id );
			$key = ( isset( $widget_option['key'] ) && !empty( $widget_option['key'] ) ) ? ( $widget_option['key'] ) : '' ;
			if(empty($widget_option)){
				$key = get_option('item_purchase_code');//Set during installer
			}
		}
	...
	public function plugins_api_result( $res, $action, $args ) {
		...
		if ( $plugin_slug == $updater_data['slug'] && $action == 'plugin_information' ){
			...
			if ( is_wp_error( $request ) ) {
				$res = new WP_Error( 'plugins_api_failed', '<p>' . __( 'An Unexpected HTTP Error occurred during the API request.', 'vibe-projects' ) . '</p><p><a href="?" onclick="document.location.reload(); return false;">' . __( 'Try again', 'vibe-projects' ) . '</a></p>', $request->get_error_message() );
			}
			...
			/* If no error, construct the data */
				...
				/* If data is empty or not an object */
					...
					$res = new WP_Error( 'plugins_api_failed', __( 'An unknown error occurred', 'vibe-projects' ), wp_remote_retrieve_body( $request ) );
	...
	public function upgrader_post_install( $true, $hook_extra, $result ) {
		...
		if ( isset( $hook_extra ) ){
			...
			if ( isset( $hook_extra['plugin'] ) && $hook_extra['plugin'] == $plugin_base ){
				...
				/* Update message */
				$fail = __( 'The plugin has been updated, but could not be reactivated. Please reactivate it manually.', 'vibe-projects' );
				$success = __( 'Plugin reactivated successfully. ', 'vibe-projects' );
	...
	public function add_dashboard_widget() {
		...
		/* Widget name */
		$widget_name = $updater_data['name'] . __( ' Plugin Updates', 'vibe-projects' );
	...
	public function dashboard_widget_callback() {
		...
		if ( !empty( $widget_option ) && is_array( $widget_option ) ){
			...
			if ( true === $updater_data['role'] ){
				...
				echo '<p>'. __( 'Username: ', 'vibe-projects' ) . '<code>' . $username . '</code></p>';

				/* activation key input */
				$key = isset( $widget_option['key'] ) ? $widget_option['key'] : '' ;
				echo '<p>'. __( 'Email: ', 'vibe-projects' ) . '<code>' . $key . '</code></p>';
			}
			else{

				/* activation key input */
				$key = isset( $widget_option['key'] ) ? $widget_option['key'] : '' ;
				echo '<p>'. __( 'Key: ', 'vibe-projects' ) . '<code>' . $key . '</code></p>';
			}


			/* if key status is valid */
			if ( $widget_option['status'] == 'valid' ){
				_e( '<p>Your plugin update is <span style="color:green">active</span></p>', 'vibe-projects' );
			}
			/* if key is not valid */
			elseif( $widget_option['status'] == 'invalid' ){
				_e( '<p>Your input is <span style="color:red">not valid</span>, automatic updates is <span style="color:red">not active</span>.</p>', 'vibe-projects' );
				echo '<p><a href="' . $edit_url . '" class="button-primary">' . __( 'Edit Key', 'vibe-projects' ) . '</a></p>';
			}
			/* else */
			else{
				_e( '<p>Unable to validate update activation.</p>', 'vibe-projects' );
				echo '<p><a href="' . $edit_url . '" class="button-primary">' . __( 'Try again', 'vibe-projects' ) . '</a></p>';
			}
		}
		/* if activation key is not yet set/empty */
		else{
			echo '<p><a href="' . $edit_url . '" class="button-primary">' . __( 'Add Purchase Code', 'vibe-projects' ) . '</a></p>';
		}
	}
	...
	public function dashboard_widget_control_callback() {
		...
		if ( isset( $_POST[$widget_id] ) && isset( $_POST['dashboard-widget-nonce'] ) && wp_verify_nonce( $_POST['dashboard-widget-nonce'], 'edit-dashboard-widget_' . $widget_id ) ){
			...
			$remote_request = array( 'timeout' => 20, 'body' => array( 'key' => ( $key ), 'login' => $username, 'autohosted' => $updater_data['autohosted'] ), 'user-agent' => 'WordPress/' . $wp_version . '; ' . $updater_data['domain'] );
			$raw_response = wp_remote_post( $remote_url, $remote_request );
		...
		<p>
			<label for="<?php echo $widget_id; ?>-username"><?php _e( 'User name', 'vibe-projects' ); ?></label>
		</p>
		<p>
			<input id="<?php echo $widget_id; ?>-username" name="<?php echo $widget_id; ?>[username]" type="text" value="<?php echo $username_option;?>"/>
		</p>
		<p>
			<label for="<?php echo $widget_id; ?>-key"><?php _e( 'Email', 'vibe-projects' ); ?></label>
		</p>
		<p>
		...
		<p>
			<label for="<?php echo $widget_id; ?>-key"><?php _e( 'Themeforest/CodeCanyon Purchase Key ( for Auto-Updates )', 'vibe-projects' ); ?></label>
		</p>
		<p>
			<input id="<?php echo $widget_id; ?>-key" class="regular-text" name="<?php echo $widget_id; ?>[key]" type="text" value="<?php echo $key_option;?>"/>
		</p>
