...
add_action( 'widgets_init', 'vibe_projects_card_burndown_widget' );
function vibe_projects_card_burndown_widget() {
    register_widget('vibe_projects_card_burndown');
}

class vibe_projects_card_burndown extends WP_Widget {

    protected static $did_script = false;
    /** constructor -- name this the same as the class above */
    function __construct() {
        $widget_ops = array( 'classname' => 'vibe_projects_card_burndown', 'description' => __('Card Burndown', 'vibe-projects') );
        $control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'vibe_projects_card_burndown' );
        parent::__construct( 'vibe_projects_card_burndown', __('DASHBOARD : Cards Burndown', 'vibe-projects'), $widget_ops, $control_ops );
    ...
    function enqueue_scripts(){
        ...
        if(apply_filters('vibebp_enqueue_profile_script',false)){
            wp_enqueue_script('chartjs-js',VIBEBP_PLUGIN_URL.'/assets/js/Chart.min.js',__FILE__,array(),VIBEPROJECTS_VERSION,true);
            wp_enqueue_script('vibe_projects_card_burndown',plugins_url('../../assets/js/cardburndown.js',__FILE__), array('wp-element', 'wp-data','chartjs-js'), VIBEPROJECTS_VERSION,true);
            wp_enqueue_style('vibe_projects_card_burndown',plugins_url('../../assets/css/cardburndown.css',__FILE__),array(),VIBEPROJECTS_VERSION);
            
            wp_localize_script('vibe_projects_card_burndown', 'cardburndown', array(
                'settings' => array(),
                'api' => rest_url(VIBE_PROJECTS_API_NAMESPACE),
                'translations' => array(
                    'total' => __('Total', 'vibe-projects'),
                    'completed' => __('Completed', 'vibe-projects'),
                    'cards' => __('Cards', 'vibe-projects'),
                    'milestones' => __('Milestones', 'vibe-projects'),
                    'not_found'=>__('No data','vibe-projects'),
    ...
    function widget( $args, $instance ) {
        ...
        echo '<div id="vibe_projects_card_burndown"></div>';
    ...
    function form($args) {  
        $defaults = array( 
            'title'  => __('Card Burndown','vibe-projects'),
