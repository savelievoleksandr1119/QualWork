...
class Vibe_Options_multi_pages extends Vibe_Options{	
	...
	*/
	function render(){
			...
			foreach($this->value['cp'] as $k=>$value){ 
				if($value != ''){
					echo '<li>
                            ...
                            <a href="javascript:void(0);" class="vibe-opts-multi-page-remove">'.__('Remove', 'micronet').'</a>
                         </li>';
				}//if
			}//foreach
		}else{
		...
		echo '<a href="javascript:void(0);" class="vibe-btn green vibe-opts-multi-page-add " rel-id="'.$this->field['id'].'-ul">'.__('Add Custom Posts', 'micronet').'</a><br/>';
